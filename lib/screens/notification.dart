              ...
              return StreamBuilder(
                  ...
                  builder: (context, taskSnapshot) {
                    ...
                    if (taskSnapshot.hasError) {
                      return Center(
                        child:
                            Text('Failed to load tasks: ${taskSnapshot.error}'),
                      );
                    }
                    ...
                    final combinedNotifications = [
                      ...pendingHostingDocs
                          .map((doc) => {'type': 'hosting', 'doc': doc}),
                      ...invitedDocs
                          .map((doc) => {'type': 'invited', 'doc': doc}),
                      ...taskDocs.map((doc) => {'type': 'task', 'doc': doc}),
                    ];
                    ...
                    return ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final doc =
                            notification['doc'] as QueryDocumentSnapshot;
                        ...
                        if (type == 'invited') {
                          ...
                          final eventName = invitedData['event_name'] ??
                              'Sự kiện không xác định';
                          final usersName = invitedData['username'] ??
                              'Sự kiện không xác định';
                          return ListTile(
                            title: Text('$usersName mời tham gia: $eventName'),
                          );
                        }
                        if (type == 'hosting') {
                          // Xử lý tài liệu từ invitedDocs
                          final hostingData =
                              doc.data() as Map<String, dynamic>;
                          final eventName = hostingData['event_name'] ??
                              'Sự kiện không xác định';
                          final usersName = hostingData['username'] ??
                              'Sự kiện không xác định';
                          return ListTile(
                            title: Text('$usersName mời tham gia: $eventName'),
                          );
                        ...
                        if (type == 'task') {
                          ...
                          final taskTitle =
                              taskData['title'] ?? 'Nhiệm vụ không xác định';
                          return ListTile(
                            title: Text('Nhiệm vụ: $taskTitle'),
                          );
                        }
                        // if (isHosting) {
                        //   final isPending =
